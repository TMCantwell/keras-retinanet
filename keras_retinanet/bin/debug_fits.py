...
def draw_box(ax, box, color, thickness=2):
    ...
    rect = patches.Rectangle((b[0],b[1]), (b[2]-b[0]), (b[3]-b[1]), linewidth=thickness, edgecolor=color, facecolor='none')
    ax.add_patch(rect)
...
def draw_boxes(ax, boxes, color, thickness=2):
    ...
    """
    for b in boxes:
        draw_box(ax, b, color, thickness=thickness)
...
def draw_caption(ax, box, caption):
    ...
    """
    b = np.array(box).astype(int)
    ax.text(b[0],b[1], caption, fontsize=14, color='red')
...
def run(generator, args, anchor_params):
    ...
    for i in range(generator.size()):
        ...
        b[:, :, 2] = 0
        ax.imshow(b)
        ax.set_title(generator.image_path(i))
        plt.show()
    return True
